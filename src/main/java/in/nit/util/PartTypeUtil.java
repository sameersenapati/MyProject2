...
@Component
public class PartTypeUtil {
	public void generatePie(String path,List<Object[]> data){
		//1.create dataset 
		DefaultPieDataset dataset=new DefaultPieDataset();
		for(Object[] arr:data) {
			//key-uomModel ,val-count
			dataset.setValue(arr[0].toString(), Double.valueOf(arr[1].toString()));
		}
		//2.create JFreeChat using ChartFactory 
		JFreeChart chart=ChartFactory.createPieChart3D("PartCodes", dataset, true, true, false);
		//3.save as Image using ChartUtils
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/PartTypeA.jpg"), chart, 600, 500);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBar(String path,List<Object[]> data) {
		...
		JFreeChart chart=ChartFactory.createBarChart("/PartCodes", "Codes", "COUNT", dataset);
		//3. Save as Image using ChartUtil
		try {
			ChartUtils.saveChartAsJPEG(new
					File(path+"/resources/images/PartTypeB.jpg"), chart, 500, 400);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
