...
public class UomTypeUtil {
	...
	public void generatePie(String path,List<Object[]> data){
		...
		JFreeChart chart=ChartFactory.createPieChart3D("UomModels", dataset, true, true, false);
		//3.save as Image using ChartUtils
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/UomA.jpg"), chart, 600, 500);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBar(String path,List<Object[]> data) {
		...
		JFreeChart chart=ChartFactory.createBarChart("UomTypeModel", "Models", "COUNT", dataset);
		//3. Save as Image using ChartUtil
		try {
			ChartUtils.saveChartAsJPEG(new
					File(path+"/resources/images/UomB.jpg"), chart, 500, 400);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
